app.controller("booking-ctrl",function($scope,$http,$timeout, $interval,$q){
	...
	$scope.initialize=function (){
		...
		$http.get("/rest/booking/CAN").then(resp=>{
			$scope.itemsCAN=resp.data;
			$scope.itemsCAN.forEach(item => {
				item.createDate = new Date(item.createDate);
			})
		})
...
//tổng time trong list
	...
	$scope.updateWFP=function (item){
		var item1=angular.copy(item);
		if (confirm("Đã thực hiện xong dịch vụ?")){
			$http.put(`/rest/booking/updateToWFP/${item1.id}`,item1).then(resp=>{
				var index = $scope.items2.findIndex(a => a.id == item1.id);
				$scope.items2[index] = item;
				alert("Đã chuyển sang chờ thanh toán !")
				$scope.initialize();
			})
			$http.delete(`/rest/delete/${item1.bookingId}`,item1)
		}
