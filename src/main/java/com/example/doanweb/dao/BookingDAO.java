...
public interface BookingDAO extends JpaRepository<Booking, Integer> {
    ...
    @Query(value = "SELECT b FROM Booking b WHERE MONTH (b.createDate) = ?1 AND YEAR (b.createDate) = ?2")
    List<Booking> findBookingWithinAMonth(int month, int year);

    @Query(value = "SELECT b FROM Booking b WHERE MONTH (b.createDate) = ?1 AND YEAR (b.createDate) = ?2 AND b.statusbooking.id='CPM'")
    List<Booking> listCPMAppointment(int month, int year);
